...
namespace StyleWatcherWin
{
    ...
    public static class ApiHelper
    {
        public static async System.Threading.Tasks.Task<string> QueryAsync(AppConfig cfg, string text)
        {
            try
            {
                using var http = new System.Net.Http.HttpClient { Timeout = System.TimeSpan.FromSeconds(Math.Max(3, cfg.timeout_seconds)) };
                var req = new System.Net.Http.HttpRequestMessage(new System.Net.Http.HttpMethod(cfg.method ?? "POST"), cfg.api_url ?? "");
                req.Content = new System.Net.Http.StringContent($"{{\"{cfg.json_key}\":\"{text?.Replace("\\","\\\\").Replace("\"","\\\"")}\"}}", System.Text.Encoding.UTF8, "application/json");
                var resp = await http.SendAsync(req);
                var raw = await resp.Content.ReadAsStringAsync();
                // 若返回 JSON 带有 msg 字段，则优先取之
                try
                {
