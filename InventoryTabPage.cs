...
namespace StyleWatcherWin
{
    ...
    /// 重要：不再对 AppConfig.Headers 做 foreach（它不是可枚举），仅按 Content_Type 单项注入请求头，避免 CS1579。
    /// </summary>
    public class InventoryTabPage : TabPage
    {
        ...
        private async Task<(bool ok, List<InvRow> rows)> FetchAndParseAsync(string styleName)
        {
            try
            {
                ...
                // 仅按 Content_Type 单项注入，避免对 Headers foreach
        ...
        private static void AttachClickTooltip(PlotView pv, List<string> colors, List<string> sizes, Dictionary<(string,string),int> map)
        {
            ...
            pv.MouseDown += (s,e) =>
            {
                ...
                var xAxis = pv.Model?.Axes?.FirstOrDefault(a => a.Position == AxisPosition.Bottom);
                var yAxis = pv.Model?.Axes?.FirstOrDefault(a => a.Position == AxisPosition.Left);
                if (xAxis == null || yAxis == null) return;
                double xVal = xAxis.InverseTransform(pt.X);
                double yVal = yAxis.InverseTransform(pt.Y);
                int ix = (int)Math.Round(xVal);
                int iy = (int)Math.Round(yVal);
